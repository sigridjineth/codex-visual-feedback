...
Options:
  --out         Output video path (default: .codex-visual-loop/record/app-window-<app>-<ts>-<pid>-<rand>.mov)
  ...
  --frames-dir  Output dir for extracted frames (default: .codex-visual-loop/record/frames-<app>-<ts>-<pid>-<rand>)
  ...
  --summary-out      Output dir for summary frames (default: .codex-visual-loop/record/summary-<video>-<ts>-<pid>-<rand>)
...
echo "cvlp: app='${process}' out='${out}' duration=${duration}s bounds=${x},${y},${w},${h}" >&2
if [[ ${extract_frames} -eq 1 ]]; then
  echo "cvlp: frames -> ${frames_dir} (fps=${fps})" >&2
fi
if [[ ${run_summary} -eq 1 ]]; then
  echo "cvlp: summary -> mode=${summary_mode} max=${summary_max}" >&2
fi
...
if [[ ${print_json} -eq 1 ]]; then
  VIDEO_PATH="${out}" FRAMES_DIR="${frames_out}" APP_NAME="${process}" APP_SLUG="${slug}" DURATION="${duration}" FPS="${fps}" X="${x}" Y="${y}" W="${w}" H="${h}" SUMMARY_JSON="${summary_json}" SUMMARY_ERROR="${summary_error}" \
  python3 - <<'PY'
import json
import os
...
payload = {
    ...
    "bounds": {
        "x": int(float(os.environ.get("X") or 0)),
        "y": int(float(os.environ.get("Y") or 0)),
